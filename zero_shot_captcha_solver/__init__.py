...
def solve_captcha(captcha_img: np.ndarray, object: str, model=None) -> np.ndarray:
    ...
    similarity_scores = text_image_similarity.compute_texts_images_similarity(
        texts=[object], images=imgs_list, clip_model=model
    )  # shape (1, image)
...
def solve_captcha_from_path(path: str, object: Optional[str] = None, model=None) -> np.ndarray:
    """Solves a captcha by comparing all the images in the grid to the text.
